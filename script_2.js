...
const app = {
  ...
  similarityScore(a, b) {
    ...
    const contain = (left.includes(right) || right.includes(left)) ? 1 : 0;
    const prefix = left.startsWith(right) || right.startsWith(left) ? 0.1 : 0;
    return Math.max(bigram * 0.50 + Math.max(0, edit) * 0.34 + contain * 0.10 + prefix, contain ? 0.74 : 0);
  },

  normalizeOCRLoose(rawText) {
  ...
  preprocessPhotoToCanvas(img, mode = 'gray', cropPreset = 'full') {
    ...
    if (cropPreset === 'center') {
      sx = srcW * 0.10; sy = srcH * 0.16; sw = srcW * 0.80; sh = srcH * 0.56;
    } else if (cropPreset === 'bottom') {
      sx = srcW * 0.06; sy = srcH * 0.34; sw = srcW * 0.88; sh = srcH * 0.50;
    }
    const maxSide = this.isLikelyMobile() ? 1280 : 1500;
    const scale = Math.min(maxSide / Math.max(sw, sh, 1), 1.6);
    const canvas = document.createElement('canvas');
    canvas.width = Math.max(520, Math.round(sw * scale));
    canvas.height = Math.max(260, Math.round(sh * scale));
    ...
    for (let i = 0; i < data.length; i += 4) {
      ...
      let value = gray;
      if (mode === 'gray') value = Math.min(255, Math.max(0, (gray - 110) * 1.85 + 128));
      else if (mode === 'bw') value = gray > 142 ? 255 : 0;
      else if (mode === 'soft') value = Math.min(255, Math.max(0, (gray - 118) * 1.45 + 128));
  ...
  getPhotoSearchTexts(rawText) {
    ...
    const loose = this.normalizeOCRLoose(base);
    const compact = this.compactComparableText(loose || base);
    const lines = Array.from(new Set(base.split(/\n+/).map(line => line.trim()).filter(Boolean)));
    const normalizedLines = Array.from(new Set((normalized + '\n' + loose).split(/\n+/).map(line => line.trim()).filter(Boolean)));
    const tokens = Array.from(new Set([...this.tokenizeComparableText(base), ...this.tokenizeComparableText(loose)]));
    const phrases = [];
  ...
  normalizePhotoOCRText(rawText) {
    return this.normalizeOCRLoose(rawText)
      .replace(/ov[e3]r\s*d[o0]s[e3]/g, 'overdose')
      .replace(/0verdose/g, 'overdose')
      .replace(/overd0se/g, 'overdose')
  ...
  getBrandCatalog() {
    ...
    const aliases = {
      'blackburn': ['black burn','blakburn','black bun','black bum','black burm','blackbun'],
  ...
  scoreBrandCandidate(entry, sourceText) {
    ...
    const textLoose = this.normalizeOCRLoose(sourceText);
    const textCompact = this.compactComparableText(textNorm || textLoose);
    const textTokens = Array.from(new Set([...this.tokenizeComparableText(textNorm), ...this.tokenizeComparableText(textLoose)]));
    let score = 0;
    if (!textCompact) return 0;
    const aliasCompacts = (entry.aliases || []).map(alias => this.compactComparableText(alias)).filter(Boolean);
    const candidates = [entry.compact, ...aliasCompacts];
    candidates.forEach(candidate => {
      if (!candidate) return;
      if (textCompact === candidate) score = Math.max(score, 620);
      if (textCompact.includes(candidate)) score = Math.max(score, 520);
      if (candidate.includes(textCompact) && textCompact.length >= 5) score = Math.max(score, 340);
      score = Math.max(score, Math.round(this.similarityScore(textCompact, candidate) * 430));
    });
    const tokenHits = (entry.tokens || []).filter(token => textTokens.includes(token)).length;
    score += tokenHits * 110;
    if ((entry.tokens || []).length === 1 && textTokens.includes(entry.tokens[0])) score += 180;
    if (entry.norm && this.getPhotoSearchTexts(textNorm).some(line => this.similarityScore(line, entry.norm) >= 0.86)) score += 120;
    return score;
  },

  getLikelyBrands(sourceText, limit = 3) {
    const ranked = this.getBrandCatalog().map(entry => ({ ...entry, score: this.scoreBrandCandidate(entry, sourceText) })).filter(item => item.score >= 180).sort((a, b) => b.score - a.score);
  ...
  getLockedBrand(sourceText) {
    ...
    const exactLine = this.getPhotoSearchTexts(sourceText).some(line => this.compactComparableText(line).includes(top.compact));
    if (top.score >= 520 || (top.score >= 430 && gap >= 90) || (exactLine && top.score >= 360)) return { ...top, gap };
    return null;
  },
  ...
  async extractPhotoTexts(imgEl) {
    ...
    const tasks = [
      { key: 'soft', label: 'Quick English OCR', crop: this.state.photoCropPreset || 'full', lang: 'eng' },
      { key: 'gray', label: 'Focused English OCR', crop: 'center', lang: 'eng' }
    ];
    const previews = [];
    const textBlocks = [];
    ...
    for (let idx = 0; idx < tasks.length; idx++) {
      ...
      this.renderPhotoPassPreviews(previews);
      this.updatePhotoStatus(`OCR-проход ${idx + 1}/${tasks.length}: ${task.label}…`, 'info');
      const result = await Tesseract.recognize(canvas, task.lang, { logger: (message) => { if (message.status === 'recognizing text') { const pct = Math.round((message.progress || 0) * 100); this.updatePhotoStatus(`OCR ${idx + 1}/${tasks.length}: ${pct}%`, 'info'); } } });
      ...
      const loose = this.normalizeOCRLoose(rawText);
      if (rawText) textBlocks.push(rawText);
      if (normalized) textBlocks.push(normalized);
      if (loose && loose !== normalized) textBlocks.push(loose);
      const mergedProbe = Array.from(new Set(textBlocks.join('\n').split(/\n+/).map(v => v.trim()).filter(Boolean))).join('\n');
      const lock = this.getLockedBrand(mergedProbe);
      if (lock) bestBrand = lock;
      const enoughTokens = this.tokenizeComparableText(mergedProbe).length >= 2;
      if (lock && (confidence >= 35 || enoughTokens)) break;
      if (idx === 0 && confidence >= 72 && enoughTokens) break;
    }
    const merged = Array.from(new Set(textBlocks.join('\n').split(/\n+/).map(v => v.trim()).filter(Boolean))).join('\n');
    return { text: merged, previews, confidence: bestConfidence, lockedBrand: bestBrand || this.getLockedBrand(merged) };
  },

  scorePhotoFlavorMatch(flavor, sourceText, brandContext = null) {
    const sourceNorm = this.normalizePhotoOCRText(sourceText);
    const sourceLoose = this.normalizeOCRLoose(sourceText);
    const sourceCompact = this.compactComparableText(sourceNorm || sourceLoose);
    const sourceTokens = Array.from(new Set([...this.tokenizeComparableText(sourceNorm), ...this.tokenizeComparableText(sourceLoose)]));
    ...
    if (brandContext && this.compactComparableText(brandContext.brand) === brandCompact) {
      score += Math.round(Math.max(320, brandContext.score * 1.05));
      reasons.push('совпал бренд');
    } else {
      const brandScore = this.scoreBrandCandidate({ brand: flavor.brand, norm: brand, compact: brandCompact, tokens: brandTokens, aliases: [] }, sourceNorm || sourceLoose);
      score += Math.round(brandScore * 0.55);
      if (brandScore >= 220) reasons.push('бренд близок');
    }
    ...
    let bestLabelSimilarity = 0; let bestNameSimilarity = 0;
    searchTexts.forEach(line => {
      bestLabelSimilarity = Math.max(bestLabelSimilarity, this.similarityScore(line, label));
      bestNameSimilarity = Math.max(bestNameSimilarity, this.similarityScore(line, name), this.similarityScore(this.compactComparableText(line), nameCompact));
    });
    score += Math.round(bestLabelSimilarity * 160 + bestNameSimilarity * 190);
    if (bestNameSimilarity >= 0.86) reasons.push('вкус почти совпал'); else if (bestNameSimilarity >= 0.68) reasons.push('вкус похож');
    ...
    score += nameHitCount * 120 + orderedNameHits * 42 + brandHitCount * 85;
    if (nameHitCount) reasons.push(`совпало слов вкуса: ${nameHitCount}/${nameTokens.length}`);
    if (!nameHitCount && bestNameSimilarity < 0.52 && !sourceCompact.includes(nameCompact)) score -= 80;
  ...
  findPhotoMatches(sourceText, limit = 10) {
    ...
    const loose = this.normalizeOCRLoose(sourceText);
    const mergedSource = [cleaned, loose].filter(Boolean).join('\n');
    if (!mergedSource || mergedSource.length < 2) return [];
    const lockedBrand = this.getLockedBrand(mergedSource);
    const likelyBrands = lockedBrand ? [lockedBrand] : this.getLikelyBrands(mergedSource, 3);
    ...
    if (lockedBrand) {
      ...
      const filtered = this.state.flavors.filter(flavor => brandSet.has(this.compactComparableText(flavor.brand)));
      if (filtered.length) candidateFlavors = filtered;
    }
    return candidateFlavors.map(flavor => {
      ...
      const match = this.scorePhotoFlavorMatch(flavor, mergedSource, brandContext);
      return { flavor, score: match.score, reasons: match.reasons, confidence: this.clampInt(Math.min(99, Math.max(18, Math.round(match.score / 9))), 0, 99), lockedBrand };
    }).filter(item => item.score >= (lockedBrand ? 120 : 145)).sort((a, b) => b.score - a.score || this.getFlavorLabel(a.flavor).localeCompare(this.getFlavorLabel(b.flavor), 'ru')).slice(0, limit);
  },
