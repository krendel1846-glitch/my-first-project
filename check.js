const app = {
  ...
  async extractPhotoTexts(imgEl) {
    ...
    const tasks = [
      { key: 'gray', label: 'Контраст + серый', crop: this.state.photoCropPreset || 'full' },
      { key: 'bw', label: 'Жесткий ч/б', crop: this.state.photoCropPreset || 'full' },
      { key: 'invert', label: 'Инверсия', crop: this.state.photoCropPreset || 'full' },
      { key: 'gray', label: 'Центр этикетки', crop: 'center' }
    ];
    const previews = [];
    const textBlocks = [];
      ...
      const result = await Tesseract.recognize(canvas, 'eng+rus', {
        logger: (message) => {
          if (message.status === 'recognizing text') {
            ...
            this.updatePhotoStatus(`OCR-проход ${idx + 1}/${tasks.length}: ${task.label} — ${pct}%`, 'info');
          }
        }
      });
      const txt = String(result && result.data && result.data.text ? result.data.text : '').trim();
  ...
  scorePhotoFlavorMatch(flavor, sourceText) {
    const sourceNorm = this.normalizeComparableText(sourceText);
    const sourceCompact = this.compactComparableText(sourceText);
    const sourceTokens = this.tokenizeComparableText(sourceText);
    const searchTexts = this.getPhotoSearchTexts(sourceText);

    const brand = this.normalizeComparableText(flavor.brand);
    const name = this.normalizeComparableText(flavor.name);
    const label = `${brand} ${name}`.trim();
    const desc = this.normalizeComparableText(flavor.description);
    const type = this.normalizeComparableText(flavor.type);
    ...
    if (sourceCompact.includes(labelCompact) && labelCompact.length >= 5) {
      score += 520;
      reasons.push('полное название найдено в OCR-тексте');
    } else if (labelCompact.includes(sourceCompact) && sourceCompact.length >= 5) {
      score += 300;
      reasons.push('OCR-подстрока близка к полному названию');
    }
    if (sourceCompact.includes(nameCompact) && nameCompact.length >= 4) {
      score += 250;
      reasons.push('название вкуса найдено напрямую');
    }
    if (sourceCompact.includes(brandCompact) && brandCompact.length >= 4) {
      score += 190;
      reasons.push('бренд найден напрямую');
    }

    let bestSimilarity = 0;
    searchTexts.forEach(line => {
      bestSimilarity = Math.max(bestSimilarity,
        this.similarityScore(line, labelCompact),
        this.similarityScore(line, nameCompact),
        this.similarityScore(line, `${brandCompact} ${nameCompact}`)
      );
    });
    score += Math.round(bestSimilarity * 220);
    if (bestSimilarity >= 0.83) reasons.push('очень близкое совпадение после OCR');
    else if (bestSimilarity >= 0.62) reasons.push('похожее написание после OCR');
    ...
    score += nameHitCount * 62 + brandHitCount * 52;
    if (nameHitCount) reasons.push(`совпало токенов вкуса: ${nameHitCount}`);
    if (brandHitCount) reasons.push(`совпало токенов бренда: ${brandHitCount}`);

    if (desc) {
      const descScore = Math.max(...searchTexts.slice(0, 10).map(line => this.similarityScore(line, desc)).concat([0]));
      score += Math.round(descScore * 50);
    }
    if (type && sourceNorm.includes(type) && type.length >= 5) score += 18;

    if ((brandCompact === 'blackburn' || brandCompact === 'blackburntobacco' || brandCompact === 'blackburnhookah') && sourceCompact.includes('blackburn')) {
      score += 90;
      reasons.push('нормализован бренд Blackburn');
    }
  ...
  findPhotoMatches(sourceText, limit = 10) {
    const cleaned = this.normalizeComparableText(sourceText);
    if (!cleaned || cleaned.length < 2) return [];
    return this.state.flavors
      .map(flavor => {
        const match = this.scorePhotoFlavorMatch(flavor, cleaned);
        return {
          flavor,
          score: match.score,
          reasons: match.reasons,
          confidence: this.clampInt(Math.min(99, Math.max(10, Math.round(match.score / 7.2))), 0, 99)
        };
      })
    ...
    container.innerHTML = matches.map((item, index) => `
      <div class="photo-match-card ${index === 0 ? 'best' : ''}">
        ...
        <div class="photo-actions" style="margin-top:12px">
          <button type="button" class="btn btn-primary btn-sm" onclick="app.addFlavorToFirstEmptySlot('${'${'}String(item.flavor.id).replace(/'/g, '&#39;')${'}'}');app.switchTab('selector', document.querySelector('.nav-btn[data-tab='selector']'))">Это он — добавить в выбор вкусов</button>
        </div>
      </div>
    `).join('');
  ...
  async analyzePhotoSearch() {
    ...
    try {
      this.updatePhotoStatus('Запускаю премиум OCR: подготовка изображения и несколько проходов распознавания…', 'info');
